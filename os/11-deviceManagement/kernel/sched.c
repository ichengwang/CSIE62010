...
void switchToFirstTask() {
	if (list_isempty((list_t*)&TCBRdy[PRIO_LEVEL-1])) {
		DEBUG("no system task can be run.");
		return;
	} else {
		taskCB_t *systemTask = (taskCB_t *)TCBRdy[PRIO_LEVEL-1].node.next;
		ctx_t *next = &systemTask->ctx;
		TCBRunning = systemTask;
		systemTask->state = TASK_RUNNING;
		switch_first(next);
	}
}
